...
public class OnlineMapsProvider
{
    ...
    private static void InitProviders()
    {
        providers = new []
        {
            ...
            new OnlineMapsProvider("skobbler", "Skobbler Maps")
            {
                _types = new []
                {
                    new MapType(MAP)
                    {
                        urlWithLabels = "http://tiles{rnd1-4}-{apikey}.skobblermaps.com/TileService/tiles/2.0/110211332100/10/{zoom}/{x}/{y}.png"
                    }
                },
                extraFields = new []
                {
                    new ExtraField("Api Key", "apikey", "bc7b4da77e971c12cb0e069bffcf2771"),
                }
            },
    ...
    public static string Upgrade(int providerID, int type)
    {
        ...
        else if (providerID == 8) builder.Append("skobbler");
